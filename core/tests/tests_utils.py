...
class UtilsTestCase(TestCase):
    ...
    # Verifica que un objeto timedelta se desglose correctamente en una tupla de (horas, minutos, segundos).
    # Valida también que lance un TypeError si se envía un tipo de dato incorrecto.
    ...
    # Verifica que la función genere un color aleatorio en formato de texto.
    # Valida que el color devuelto exista dentro de la lista constante de colores permitidos.
    def test_random_color(self):
        color = utils.random_color()
        self.assertIsInstance(color, str)
        self.assertIn(color, utils.COLORS)

    # Verifica el cálculo de duración entre dos fechas/horas con distintas zonas horarias.
    # Valida que la diferencia devuelva un timedelta exacto compensando los distintos husos horarios.
